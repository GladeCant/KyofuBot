...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (item.requiredLevel > client.rpg.calculateLevel(user.xp)[0]) return message.inlineReply(`<:unchecked:860839603098877953> • Vous n'avez pas le niveau requis pour acheter l'objet **${item.name}**. (requis : 👑 **${item.requiredLevel}**)`);

    const userItems = user.items;
    userItems.push(item.name);

    await client.rpg.updateUser(message.author, { coins: user.coins - item.price });
    await client.rpg.updateUser(message.author, { items: userItems });
    if (item.stock !== 0 && item.stock !== -1) await client.rpg.updateItem(item.name, { stock: item.stock - 1 });
